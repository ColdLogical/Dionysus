...
class WebOperationsTests: XCTestCase {
        ...
        let workingTitleId = "EP017390090029"
        ...
        func testFetchTitleDetails() {
                //This is an integration test
                //  Set default config path so the operation hits real services
                WebOperations.setConfiguration(WebOperations.defaultConfigPath())
                
                //Make a channel to work with for the integration test
                let channel = Channel.newChannel()
                channel.setValue(workingTitleId, forKey: kTitleId)
                
                //Operation should return a success
                var connectExp = expectationWithDescription("Fetch Title Details Success Test")
                func success(request: NSURLRequest, successful: Bool!) {
                        XCTAssert(successful == true, "Did not successfully get title details")
                        
                        let channelDescription = channel.valueForKey(kTitleDescription) as? String
                        XCTAssert(channelDescription! == "Jake discovers the woman he hit it off with at the bar is the lawyer representing a perp he's trying to put in jail; Gina and Charles try to leave their relationship in the past.", "Channel description should match description received from services")
                        
                        let channelImageUri = channel.valueForKey(kTitleImage) as? String
                        XCTAssert(channelImageUri!.hasSuffix("tvbanners/v8/NowShowing/10909515/p10909515_b1t_v8_aa.jpg"), "Image URI must be smallest width image uri for title")
                ...
                WebOperations.fetchTitleDetails(channel, completion: success, failure: failure)
                
                waitForExpectationsWithTimeout(60) { (error: NSError!) in
                        XCTAssert(true, "Fetch Title Details timed out")
                }
        }
        ...
        func testTitleDetailsURL() {
                let url = WebOperations.titleDetailsURL("yourSistersDetails")
                XCTAssert(url == (WebOperations.baseURL() + kTitleDetailsEndpoint + "/yourSistersDetails"), "Title details URL must be combination of baseURL, title details endpoint, and title id")
        }
        ...
        func testTune() {
                ...
                WebOperations.tune(workingChannel, deviceMacAddress: workingDevice, completion: success, failure: failure)
